 ...
 * Date: 2017/3/27
 */
class PublicesController extends ApiController {
...
//7>生成随机的字母和数字
        ...
        imagepng($image,IMAGECODE_PATH.$session_id.".png");
        //4>imagedestroy() 结束图形函数 销毁$image
        imagedestroy($image);
        if(file_exists(IMAGECODE_PATH.$session_id.".png")){
            $code_url = IMAGECODE_URL.$session_id.".png";
            echo json_encode(array("code" => 1000, "data" => $code_url, "msg" =>"y验证码获取成功","session_id"=>$session_id ));
            exit();
        }else{
            echo json_encode(array("code" => 1004, "data" => "", "msg" =>"验证码获取失败" ));
            exit();
        }
    }
