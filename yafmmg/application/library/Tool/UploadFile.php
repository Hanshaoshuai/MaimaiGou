        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    ...
                    //把手机端的图片放到app文件夹里
                    $thumbPath_ = $thumbPath;
                    if(in_array($thumbPrefix[$i], array('366x366_','750x750_'))){
                        $thumbPath_ = UPLOAD_PATH.$this->subDir;
                        if(!is_dir($thumbPath_)) {
                            mkdir($thumbPath_,0777,true);
                        }
                    }

                    if(1 == $this->thumbType){
                        Image::thumb2($filename,$thumbPath_.$thumbname.'.'.$thumbExt,'',$thumbWidth[$i],$thumbHeight[$i],true);
                    }else{
                        Image::thumb($filename,$thumbPath_.$thumbname.'.'.$thumbExt,'',$thumbWidth[$i],$thumbHeight[$i],true);                        
                    }
