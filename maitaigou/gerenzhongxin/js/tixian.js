function Tixiang() {
	...
	function　 Tixian() { //请求获取麦豆
		...
		function tanchu() {
			$(".loding").css({
				"display": "block"
			}).delay(5000).click(function tiaozhuan() {
		...
		//传入jq对象
		function val(dom,string) {
			dom.on('focus', function() {
				if($(this).val() == string){
					$(this).addClass('value_a');
					$(this).val('');
				}
			})
	...
	function dong() {
		if(wodeyejizhishu !== null) {
			...
			$(".highcharts-data-labels .highcharts-label span").css({
				"color": "#c11920"
			}) //字的颜色
			//		    }, 2000);
			$(".highcharts-data-labels .highcharts-label span").css({
			...
			$(".highcharts-data-labels .highcharts-label .ree").css({
				"color": "#000000"
			}) //字的颜色
			...
			$("text").eq(0).children().remove();
			$("text").eq(2).children().remove();
			$("text").eq(4).children().remove();
			$("text").eq(6).children().remove();
			$("text").eq(8).children().remove();
			$("text").eq(10).children().remove();
		}
	};
